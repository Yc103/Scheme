...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        talking = LambdaProcedure(formals, body, env)
        env.define(signature.first, talking)
...
def do_lambda_form(expressions, env):
    ...
    # BEGIN PROBLEM 7
    "*** YOUR CODE HERE ***"
    people = expressions.rest
    return LambdaProcedure(formals,people,env)
    # END PROBLEM 7
...
def do_and_form(expressions, env):
    ...
    while expressions != nil:
        FAM = scheme_eval(expressions.first, env)
        if is_scheme_false(FAM):
            return FAM
        else:
...
def do_or_form(expressions, env):
    ...
    if expressions is nil:
        return False
    while expressions != nil:
        TAM = scheme_eval(expressions.first, env)
        if is_scheme_true(TAM):
            return TAM
        else:
            expressions = expressions.rest
    return TAM
...
def make_let_frame(bindings, env):
    ...
    "*** YOUR CODE HERE ***"
    while bindings:
        nofun = bindings.first 
        vals = Pair(eval_all(nofun.rest,env), vals)
        names = Pair(nofun.first, names)
        bindings = bindings.rest
        validate_form(nofun,2,2)
...
def do_mu_form(expressions, env):
    ...
    stuff = expressions.rest
    #print("hi")
    return MuProcedure(formals,stuff)
    # END PROBLEM 11
